  ...
  for(var i = 0; i < map.length ; i++)
  {
    ...
    for(var j = 0; j < map[i].length; j++)
    {
      if(locationName == map[i][j])
      {
        ...
        if (map[i][j-1])
          {
            location.choices["Go west"] = {goto: map[i][j-1]};
          }
        if (map[i-1] && map[i-1][j])
          {
            location.choices["Go north"] = {goto: map[i-1][j]};
          }
        if (map[i+1] && map[i+1][j])
          {
            location.choices["Go south"] = {goto: map[i+1][j]};
          }
        if (map[i][j+1])
          {
            location.choices["Go east"] = {goto: map[i][j+1]};
          }
        if (map[i-1] && map[i-1][j-1])
          {
            location.choices["Go north west"] = {goto: map[i-1][j-1]};
          }
        if (map[i-1] && map[i-1][j+1])
          {
            location.choices["Go north east"] = {goto: map[i-1][j+1]};
          }
        if (map[i+1] && map[i+1][j-1])
          {
            location.choices["Go south west"] = {goto: map[i+1][j-1]};
          }
        if (map[i+1] && map[i+1][j+1])
          {
            location.choices["Go south east"] = {goto: map[i+1][j+1]};
          }
      }
    }
  }
